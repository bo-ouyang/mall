...
class OrderModel extends Model {
        public function getOrderId(){
            ...
            $oid = $this->where(array('order_id'=>$order_id))->find()?$this->getOrderId():$order_id;
            return $oid;
        }
        //查订单
        public function getOrderByUserId($user_id,$order_id=''){
            if($order_id!=''){
                $og['order'] = $this->where(['user_id'=>$user_id,'order_id'=>$order_id])->find();
        ...
        public function getOrderGoods($order_id){
            ...
            $og = Db::name('order_goods')->field($field)->where(['order_id'=>$order_id])->select();
            foreach ($og as $k=>$v){
